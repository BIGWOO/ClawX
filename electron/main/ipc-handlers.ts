  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[agent-i-validate] validating provider type: ${providerType}`);
...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[agent-i-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[agent-i-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.error?.includes('API error: 404')) {
    console.log(
      `[agent-i-validate] ${providerType} /models returned 404, falling back to /chat/completions probe`
    );
